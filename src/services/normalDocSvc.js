...
export function normalDocsSvc(explainSvc) {
  var entityMap = {
    '&': '&',
    '<': '<',
    '>': '>',
  ...
  var assignEmbeds = function(normalDoc, doc, fieldSpec) {
    Object.keys(fieldSpec.embeds).forEach(function (key) {
      var embedField = fieldSpec.embeds[key];
      normalDoc.embeds[embedField] = doc[embedField];
    });
  };

  var assignTranslations = function(normalDoc, doc, fieldSpec) {
    Object.keys(fieldSpec.translations).forEach(function (key) {
      var translationField = fieldSpec.translations[key];
      normalDoc.translations[translationField] = doc[translationField];
    });
  };
  
  var assignUnabridgeds = function(normalDoc, doc, fieldSpec) {
    Object.keys(fieldSpec.unabridgeds).forEach(function (key) {
      var unabridgedField = fieldSpec.unabridgeds[key];
  ...
  var assignSubs = function(normalDoc, doc, fieldSpec) {
    ...
    if (fieldSpec.subs === '*') {
      Object.keys(doc).forEach(function(value, fieldName) {
      ...
      Object.keys(fieldSpec.subs).forEach(function(subFieldName) {
      ...
      Object.keys(fieldSpec.functions).forEach(function(functionField) {
      ...
      Object.keys(fieldSpec.highlights).forEach(function(hlField) {
  ...
  var NormalDoc = function(fieldSpec, doc) {
    ...
    Object.keys(this.subs).forEach(function(subValue, subField) {
  ...
  var snippitable = function(doc) {
    ...
    doc.subSnippets = function(hlPre, hlPost) {
      if (lastHlPre !== hlPre || lastHlPost !== hlPost) {
        var displayFields = Object.assign({}, doc.subs);

        Object.keys(displayFields).forEach(function(subFieldValue, subFieldName) {
  ...
  var explainable = function(doc, explainJson) {
    ...
    doc.hotMatchesOutOf = function(maxScore) {
      ...
      if (hotOutOf.length === 0) {
        Object.keys(hotMatches.vecObj).forEach(function(value, key) {
