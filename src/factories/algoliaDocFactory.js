...
export function AlgoliaDocFactory(DocFactory) {
  const Doc = function(doc, options) {
    ...
    // Handle array fields - if field has only one element, extract that element
    Object.keys(self.fieldsProperty()).forEach(function(fieldName) {
      const fieldValue = self.fieldsProperty()[fieldName];
